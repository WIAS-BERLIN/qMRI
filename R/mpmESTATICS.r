          ...
          if (mpmdata$model == 2) {
            ...
            if ((sum(ivec[xmat[, 1] == 1]) == 0) |
              ...
              th <- c(ivec[indT1] * exp(-xmat[indT1, 3] * R2star), # par[1]
                      ...
                      R2star)                                      # par[3]
              res <- try(nls(ivec ~ estatics2(par, xmat),
                             start = list(par = th),
                             control = list(maxiter = 200,
                                            warnOnly = TRUE)))
              if(class(res) == "try-error" || !res$convInfo$isConv || any(coefficients(res) < 0))
...
estimateESTATICSQL <- function(mpmdata,
  ...
  # test availability of sigma and L
  if (is.null(sigma) | is.null(L)) stop("Required arguments (sigma, L) missing without default!")
  if (length(sigma) == 1) {
    ...
    # dimensions of the data and variance estimate dont match
  ...
  # sigma has to be scaled by dataScale in the same way as the data itself
  sigma <- sigma/dataScale
  # this is the constant in the expectation value function of the non-central chi
  # that does only depend on sigma and L but not on the data:
  # see JASA 111(516), 1-11 Eq.(3)
  CL <- sigma * sqrt(pi/2) * gamma(L + 0.5) / gamma(L) / gamma(1.5)

  # starting value for R2* value
  R2star <- 0.05 * TEScale
  indT1 <- order(mpmdata$TE[as.logical(xmat[, 1])])[1]
  if (mpmdata$model == 2) {
    indMT <- order(mpmdata$TE[as.logical(xmat[, 2])])[1] + sum(xmat[, 1])
