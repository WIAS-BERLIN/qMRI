...
<<1>>=
library(qMRI)
IRinf <- readNIfTI(file.path(dataDir0,"Brainweb_IRInf"))
...
<<7>>=
IRmix <- smoothIRSolid(IRmix, verbose=FALSE)
@

and then re-estimate the fluid proportion $f$

<<8>>=
IRmix <- estimateIRsolidfixed(IRmix, verbose=FALSE)
@
...
rimage(segm[,,2])
title("Segmentation")
rimage(IRmix$Sx[,,2],zlim=c(250,500))
title("solid intensity map")
rimage(IRmix$Rx[,,2],zlim=c(0,.0015))
title("solid relaxation rate map")
rimage(IRmix$fx[,,2],zlim=c(0,.4))
...
<<10, eval=FALSE>>=
IRmix <- estimateIR(IRdata, method="QL")
@
