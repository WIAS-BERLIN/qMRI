...
@techreport{PoPaTa18,
	...
	Bdsk-Url-3 = {http://www.wias-berlin.de/preprint/2432/wias%5C_preprints_2432.pdf}}
...
maps were used to generate the artifical data on a selected (small) subcube with Rician 
...
of $T_1$ weigthed images, 8 echos of proton density ($P\!D$) weigthed images and 
6 images from a dual excitation FLASH magnetization transfer ($M\!T$) weighted 
sequence. Each echo in the image volumes is ``acquired'' at an echo time, repition time 
...
Therefore the paramter estimation can alternatively be performed by 
<<6>>=
...
which in this case of artifical data is explicitely known. 
...
The preceeding code also applies the adaptive smoothing not only to the 
